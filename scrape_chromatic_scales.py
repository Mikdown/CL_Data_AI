...
# Find the Chromatic Scales section
chromatic_scales = []

# Find the h2 header with "Chromatic Scales"
h2_tag = soup.find('h2', string="Chromatic Scales")
...
# Create dataframe
df = pd.DataFrame(chromatic_scales)

print(f"Found {len(df)} Chromatic Scales:")
print(df)

# Save to CSV in root folder
csv_path = os.path.expanduser('~/chromatic_scales.csv')
df.to_csv(csv_path, index=False)
print(f"\nSuccessfully saved {len(df)} chromatic scales to {csv_path}")
