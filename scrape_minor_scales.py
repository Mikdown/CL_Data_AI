...
# Find the Minor Scales section
minor_scales = []

# Find the h2 header with "Minor Scales"
h2_tag = soup.find('h2', string="Minor Scales")
...
if h2_tag:
    ...
    for link in all_links:
        ...
        try:
            ...
            scale_response = requests.get(scale_url, timeout=15)
...
# Create dataframe
df = pd.DataFrame(minor_scales)

print(f"Found {len(df)} Minor Scales:")
print(df)

# Save to CSV in root folder
csv_path = os.path.expanduser('~/minor_scales.csv')
df.to_csv(csv_path, index=False)
print(f"\nSuccessfully saved {len(df)} minor scales to {csv_path}")
