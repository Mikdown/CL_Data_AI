...
# Find the Major Scales section
major_scales = []

# Find the h2 header with "Major Scales"
h2_tag = soup.find('h2', string="Major Scales")
...
if h2_tag:
    ...
    for link in all_links:
        # Stop if we encounter another section header (h2 or h3)
        # Check if there's an h2 or h3 between the h2_tag and this link
        next_header = link.find_previous(['h2', 'h3'])
        if next_header and next_header != h2_tag:
            break
...
# Create dataframe
df = pd.DataFrame(major_scales)

print(f"Found {len(df)} Major Scales:")
print(df)

# Save to CSV in root folder
csv_path = os.path.expanduser('~/major_scales.csv')
df.to_csv(csv_path, index=False)
print(f"\nSuccessfully saved {len(df)} major scales to {csv_path}")
