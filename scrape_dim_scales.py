...
# Find the Diminished Scales section
diminished_scales = []

# Find the h2 header with "Diminished Scales"
h2_tag = soup.find('h2', string="Diminished Scales")
...
# Create dataframe
df = pd.DataFrame(diminished_scales)

print(f"Found {len(df)} Diminished Scales:")
print(df)

# Save to CSV in root folder
csv_path = os.path.expanduser('~/diminished_scales.csv')
df.to_csv(csv_path, index=False)
print(f"\nSuccessfully saved {len(df)} diminished scales to {csv_path}")
