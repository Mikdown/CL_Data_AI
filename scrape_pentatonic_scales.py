...
# Find the Pentatonic Scales section
pentatonic_scales = []

# Find the h2 header with "Pentatonic Scales"
h2_tag = soup.find('h2', string="Pentatonic Scales")
...
# Create dataframe
df = pd.DataFrame(pentatonic_scales)

print(f"Found {len(df)} Pentatonic Scales:")
print(df)

# Save to CSV in root folder
csv_path = os.path.expanduser('~/pentatonic_scales.csv')
df.to_csv(csv_path, index=False)
print(f"\nSuccessfully saved {len(df)} pentatonic scales to {csv_path}")
