...
# Find the h2 header with "Modes"
h2_tag = soup.find('h2', string="Modes")
...
print(f"Found {len(df)} Modes:")
print(df)

# Save to CSV in root folder
csv_path = os.path.expanduser('~/modes.csv')
df.to_csv(csv_path, index=False)
print(f"\nSuccessfully saved {len(df)} modes to {csv_path}")
